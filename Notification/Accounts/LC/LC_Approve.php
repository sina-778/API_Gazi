...
function postMethod($requestData)
{
    ...
    //temp = #spsql(zabsp_apvprcs,#id,#user,#position,xreqnum,0,xstatus,"LC Opening Cost Approval") 
    //temp = #spsql(zabsp_apvprcs,#id,#user,#position,xreqnum,0,xstatus,"Duty and Other Payment Approval") 
    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(1, $zid, PDO::PARAM_INT);
        $stmt->bindParam(2, $user, PDO::PARAM_STR);
        $stmt->bindParam(3, $xposition, PDO::PARAM_STR);
        $stmt->bindParam(4, $xreqnum, PDO::PARAM_STR);
