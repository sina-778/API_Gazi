...
function postMethod($requestData)
{
    ...
    $xadjnum = $requestData["xadjnum"];
    $ypd = 0;
    $xnote = $requestData["xnote"];
    $approvalType = 'Loan Adjustment';

    $sql = "UPDATE acloanadj set xnote1 = :xnote WHERE xadjnum = :xadjnum and zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xadjnum', $xadjnum, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xadjnum, PDO::PARAM_STR);
