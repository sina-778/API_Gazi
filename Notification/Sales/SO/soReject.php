...
function postMethod($requestData)
{
    ...
    // #sesql("update opsoheader set xnote1 ='"+xnote1+"' where zid='"+#id+"' and xsonumber='"+xsonumber+"'")
    // #spsql(zabsp_Reject_Request,#id,#user,#position,0,xsonumber,"SO")
    $sql = "UPDATE opsoheader SET xnote1 = :xnote WHERE xsonumber = :xsonumber and zid = :zid ";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xsonumber', $xsonumber, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xsonumber, PDO::PARAM_STR);
