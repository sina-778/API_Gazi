...
function postMethod($requestData)
{
    ...
    $xcrnnum = $requestData["xcrnnum"];
    $ypd = 0;
    $xnote = $requestData["xnote"];
    $approvalType = 'SLR';

    $sql = "UPDATE opcrnheader SET xnote1 = :xnote WHERE xcrnnum = :xcrnnum and zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xcrnnum', $xcrnnum, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xcrnnum, PDO::PARAM_STR);
