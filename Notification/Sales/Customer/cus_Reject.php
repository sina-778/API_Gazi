...
function postMethod($requestData)
{
    ...
    // temp = #sql("update cacus set xnote1='"+xnote1+"' where xcus='"+xcus+"'")
    // temp =  #spsql(zabsp_Reject_Request,#id,#user,#position,0,xcus,"Customer Approval")

    $sql = "UPDATE cacus SET xnote1 = :xnote WHERE xcus = :xcus and zid = :zid ";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xcus', $xcus, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xcus, PDO::PARAM_STR);
