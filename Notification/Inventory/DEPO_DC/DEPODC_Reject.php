...
function postMethod($requestData)
{
    ...
    $xdocnum = $requestData["xdocnum"];
    $ypd = 0;
    $xnote = $requestData["xnote"];
    $approvalType = 'Depo DC Approval';


    $sql = "UPDATE imdcheader SET xnote1 = :xnote WHERE xdocnum = :xdocnum and zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xdocnum', $xdocnum, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xdocnum, PDO::PARAM_STR);
