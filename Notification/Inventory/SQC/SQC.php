...
ISNULL((select xlong from zstatus where xnum = pogrnheader.xstatusgrn and zid =pogrnheader.zid), '') AS statusgrndesc,
ISNULL(xwh, '') AS xwh,
...
FROM pogrnheader
WHERE 
left(xgrnnum,3)='SQC' AND  xstatusgrn = '1' 
AND xstatusdoc not in ('1','4','7')
AND zid = ? 
AND (xsuperiorsp = ? OR xsuperior2 = ? OR xsuperior3 = ?) 
ORDER BY xgrnnum";
