...
function postMethod($requestData)
{
    ...
    $xgrnnum = $requestData["xgrnnum"];
    $ypd = 0;
    $xnote = $requestData["xnote"];
    $approvalType = 'GRN';

    $sql = "UPDATE pogrnheader SET xnote1 = :xnote WHERE xgrnnum = :xgrnnum and zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xgrnnum', $xgrnnum, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xgrnnum, PDO::PARAM_STR);
