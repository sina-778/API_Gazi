...
function postMethod($requestData)
{
    ...
    $xbomkey = $requestData["xbomkey"];
    $ypd = 0;
    $xnote = $requestData["xnote"];
    $approvalType = 'BOM';

    $sql = "UPDATE bmbomheader SET xnote1 = :xnote WHERE xbomkey = :xbomkey and zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xbomkey', $xbomkey, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xbomkey, PDO::PARAM_STR);
