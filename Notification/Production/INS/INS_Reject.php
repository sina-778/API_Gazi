...
function postMethod($requestData)
{
    ...
    // set temp =#sql("update imtorheader set xnote ='"+xnote+"' where xtornum='"+xtornum+"'")
    // set temp =  #spsql(zabsp_Reject_Request,#id,#user,#position,0,xtornum,"FINS")

    $sql = "UPDATE imtorheader SET xnote = :xnote WHERE xtornum = :xtornum and zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xtornum', $xtornum, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xtornum, PDO::PARAM_STR);
