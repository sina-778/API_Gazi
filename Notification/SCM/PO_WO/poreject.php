...
function postMethod($requestData)
{
    ...
    $xpornum = $requestData["xpornum"];
    $ypd = 0;
    $xnote = $requestData["xnote"];
    $approvalType = 'PO';

    $sql = "UPDATE poordheader SET xnote1 = :xnote WHERE xpornum = :xpornum AND zid = :zid";

    try {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':xnote', $xnote, PDO::PARAM_STR);
        $stmt->bindParam(':xpornum', $xpornum, PDO::PARAM_STR);
        ...
        $stmt1->bindParam(5, $xpornum, PDO::PARAM_STR);
