...
function postMethod($requestData)
{
    ...
    $approvalType = 'Hourly Leave Approval';
    ...
    try {
        ...
        $sql1 = "EXEC zabsp_apvprcs_HRM ?, ?, ?, ?, ?, ?, ?";
        $stmt1 = $conn->prepare($sql1);
        $stmt1->bindParam(1, $zid, PDO::PARAM_INT);
        $stmt1->bindParam(2, $user, PDO::PARAM_STR);
        $stmt1->bindParam(3, $xposition, PDO::PARAM_STR);
        $stmt1->bindParam(4, $xstaff, PDO::PARAM_STR);
        $stmt1->bindParam(5, $xyearperdate, PDO::PARAM_STR);
        $stmt1->bindParam(6, $xstatus, PDO::PARAM_STR);
